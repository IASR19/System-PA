function afterTaskComplete(colleagueId,nextSequenceId,userList){
	...
	if(sequenceId == Activity.LANCAR_NOTA && nextSequenceId == Activity.AVALIAR_GERENTE_AREA){
	...
	if(sequenceId == Activity.AVALIAR_DIRETOR_AREA && nextSequenceId == Activity.AVALIAR_CONTROLADORIA){
		hAPI.setCardValue("_rd_aprov_diretor_area","Aprovado");
		hAPI.setCardValue("aux_aprov_diretor_area","Aprovado");
	}
	
	if(sequenceId == Activity.AVALIAR_CONTROLADORIA && nextSequenceId == Activity.AVALIAR_GERENTE_FINANCEIRO){
		hAPI.setCardValue("_rd_aprov_controladoria","Aprovado");
		hAPI.setCardValue("aux_aprov_controladoria","Aprovado");
	}
	
	if(sequenceId == Activity.AVALIAR_GERENTE_FINANCEIRO && nextSequenceId == Activity.AVALIAR_DIRETOR_FINANCEIRO){
		hAPI.setCardValue("_rd_aprov_gerente_financeiro","Aprovado");
		hAPI.setCardValue("aux_aprov_gerente_financeiro","Aprovado");
	}
	
	if(sequenceId == Activity.AVALIAR_DIRETOR_FINANCEIRO && nextSequenceId == Activity.AVALIAR_PRESIDENTE){
	...
	if(sequenceId == Activity.AVALIAR_GERENTE_FINANCEIRO && nextSequenceId == Activity.GATEWAY_CORRIGIR){
		hAPI.setCardValue("_rd_aprov_gerente_financeiro","Reprovado");
