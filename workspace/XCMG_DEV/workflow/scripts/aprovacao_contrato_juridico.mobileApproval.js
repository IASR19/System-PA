...
function processMobileApproval(){
	...
	if(isApprovalActivity(NUM_STATE)){
		...
		if((targetStatus == "Reprovado" || new_feedback.status == "Reprovado")
			 && isEmpty(new_feedback.obs)){
...
function isApprovalActivity(currentState){
	return currentState == Activity.AVALIAR_GER_AREA
		|| currentState == Activity.AVALIAR_CONTROLADORIA
		|| currentState == Activity.AVALIAR_GER_FINANCEIRO
		|| currentState == Activity.AVALIAR_DIR_FINANCEIRO
...
function getApprovalFeedback(currentState){
	...
	if(currentState == Activity.AVALIAR_GER_AREA){
		...
		return { 
			"status": hAPI.getCardValue("rd_aprov_presidente")
			, "obs": hAPI.getCardValue("ds_obs_presidente")
		}
	}
	return null;
}
...
function setApprovalFeedback(currentState, feedback){
	if(currentState == Activity.AVALIAR_GER_AREA){
		...
		hAPI.setCardValue("rd_aprov_presidente", feedback.status);
		hAPI.setCardValue("ds_obs_presidente", feedback.obs);
	}
}