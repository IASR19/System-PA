function enableFields(form) {
	...
	if(activity == Activity.ABERTURA || activity == Activity.INICIO) {
		...
		fieldsEnabled.push("rd_lancado"); fieldsEnabled.push("ds_obs_lancar_nota", "aux_lancado");
		disableAllFields(form);	
	}else if(activity == Activity.AVALIAR_GERENTE_AREA){
		fieldsEnabled.push("rd_aprov_gerente_area", "ds_obs_gerente_area", "aux_aprov_gerente_area");
		disableAllFields(form);
	}else if(activity == Activity.AVALIAR_DIRETOR_AREA){
		fieldsEnabled.push("rd_aprov_diretor_area", "ds_obs_diretor_area", "aux_aprov_diretor_area");
		disableAllFields(form);
	}else if(activity == Activity.AVALIAR_CONTROLADORIA){
		fieldsEnabled.push("rd_aprov_controladoria", "ds_obs_controladoria",  "aux_aprov_controladoria" );
		disableAllFields(form);
	}else if(activity == Activity.AVALIAR_GERENTE_FINANCEIRO){
		fieldsEnabled.push("rd_aprov_gerente_financeiro", "ds_obs_gerente_financeiro", "aux_aprov_gerente_financeiro");
		disableAllFields(form);
	}else if(activity == Activity.AVALIAR_DIRETOR_FINANCEIRO){
		fieldsEnabled.push("rd_aprov_diretor_financeiro", "ds_obs_diretor_financeiro", "aux_aprov_diretor_financeiro");
		disableAllFields(form);
	}else if(activity == Activity.AVALIAR_PRESIDENTE){
		fieldsEnabled.push("rd_aprov_presidencia", "ds_obs_presidencia", "aux_aprov_presidencia");
		disableAllFields(form);
	...
	fieldsEnabled.push("aux_aprov_diretor_area");
	fieldsEnabled.push("aux_aprov_controladoria" );
	fieldsEnabled.push("aux_aprov_gerente_financeiro");
