let APPROVERS = {
	...
	*/
	...
	, setCostCenterApprovers: function(data){
		...
		$("#nm_funcao_2").val("Diretor da Area");
	...
	, setGenericApprovers: function(data){
		...
		$("#nm_funcao_3").val("Jurídico 法律部门");
		data = getUsersByGroup("juridico");		
		if(data.length > 0) {
			$("#cd_advogado").val(data[0].cd);
			$("#nm_advogado").val(data[0].nm);
		}
		
		$("#nm_funcao_4").val("Controladoria 控制部");
		data = getUsersByGroup("controladoria");		
		if(data.length > 0) {
			$("#cd_controladoria").val(data[0].cd);
			$("#nm_controladoria").val(data[0].nm);
		}
		
		$("#nm_funcao_5").val("Gerente Financeiro 財務經理");
		data = getUsersByGroup("financeiro_gerencia");
		if(data.length > 0) {
			$("#cd_gerente_financeiro").val(data[0].cd)
			$("#nm_gerente_financeiro").val(data[0].nm);
		}
		
		$("#nm_funcao_6").val("Diretor Financeiro 首席財務官");
		data = getUsersByGroup("financeiro_diretoria");
		if(data.length > 0) {
		...
		data = getUsersByGroup("presidencia");
		if(data.length > 0) {
			$("#cd_presidente").val(data[0].cd);
			$("#nm_presidente").val(data[0].nm);
		}
	}
