	...
	, displayForm : function() {
		...
		if(CONTEXT.MODE == "VIEW"){
			...
			if (aux_aprov_gerente_financeiro != ""){
				$("input[name=rd_aprov_ger_fin][value=" + aux_aprov_gerente_financeiro + "]").prop('checked', true);
			}
			...
			var aux_aprov_presidencia = $("#aux_aprov_presidente").val();
			if (aux_aprov_presidencia != ""){
				$("input[name=rd_aprov_presidente][value=" + aux_aprov_presidencia + "]").prop('checked', true);				
			}
			...
			if (aux_aprov_gerente_financeiro != ""){
				$("input[name^=_rd_aprov_ger_fin][value=" + aux_aprov_gerente_financeiro + "]").prop('checked', true);
			}
			...
			var aux_aprov_presidencia = $("input[name^=aux_aprov_presidente]").val();
			if (aux_aprov_presidencia != ""){
				$("input[name^=_rd_aprov_presidente][value=" + aux_aprov_presidencia + "]").prop('checked', true);	
			}
	...
	, updateTotalCost: function(idx){
		if(idx != undefined && idx != null){
			...
			$totais.each(function(idx, el){
				...
				console.log(('valor total'+idx)+ UTILS.parseFloat($(el).val()))
			});
...
function escolheSelect(nome_select) {

	var id_select = nome_select.name; 
	var value_select = nome_select.value;
	
	 if(id_select == "nm_estado_civil_1" && value_select == "CASADO"){
		 document.getElementById("div_conjuge_1").style.display = "block";
	 }
		 else if (id_select == "nm_estado_civil_1" && value_select != "CASADO"){
			 document.getElementById("div_conjuge_1").style.display = "none";
		 }
	 
	 if(id_select == "nm_estado_civil_2" && value_select == "CASADO"){
		 document.getElementById("div_conjuge_2").style.display = "block";
	 }
		 else if (id_select == "nm_estado_civil_2" && value_select != "CASADO"){
			 document.getElementById("div_conjuge_2").style.display = "none";
		 }
	   
	 if(id_select == "nm_estado_civil_3" && value_select == "CASADO"){
		 document.getElementById("div_conjuge_3").style.display = "block";
	 }
		 else if (id_select == "nm_estado_civil_3" && value_select != "CASADO"){
			 document.getElementById("div_conjuge_3").style.display = "none";
		 }
	 
	 if(id_select == "nm_estado_civil_4" && value_select == "CASADO"){
		 document.getElementById("div_conjuge_4").style.display = "block";
	 }
		 else if (id_select == "nm_estado_civil_4" && value_select != "CASADO"){
			 document.getElementById("div_conjuge_4").style.display = "none";
		 }
}
...
function Contrato() {
	...
	if (tipo_contrato == "EMPREITADA"){
			
		window.open('https://xcmgbrasil124786.fluig.cloudtotvs.com.br/portal/p/1/ecmnavigation?app_ecm_navigation_doc=90626&solicitacao='+id,'','width=1000,height=600');
		
	}
	else if (tipo_contrato == "PRESTACAO_SERVICOS"){
	
		window.open('https://xcmgbrasil124786.fluig.cloudtotvs.com.br/portal/p/1/ecmnavigation?app_ecm_navigation_doc=90629&solicitacao='+id,'','width=1000,height=600');
