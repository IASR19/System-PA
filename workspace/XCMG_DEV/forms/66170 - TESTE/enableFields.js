...
function enableFields(){
	...
	log.info("CONTEXT.MODE: " + CONTEXT.MODE);
	...
	if(CONTEXT.CURRENT_STATE != Activity.ZERO && CONTEXT.CURRENT_STATE != Activity.INICIO) {
		...
		if ($("#nm_categoria").val() == "DOCUMENTACAO_PROCESSO"){
			$('#div-tb-documento-processo').show();
		}else{
			$('#div-tb-alteracaobom').show();
		}
	}else{
		$('#bt_add_doc').show();
		$('#bt_add_bom').show();
	...
	if(	CONTEXT.CURRENT_STATE == Activity.APROVA_ENG_PROCESSO || 
		CONTEXT.CURRENT_STATE == Activity.APROVA_ENG_PRODUTO ||
		CONTEXT.CURRENT_STATE == Activity.INICIO ||
		CONTEXT.CURRENT_STATE == Activity.ZERO 
		) {
		$('#div-analise-revisor').show();			
	}
	
	if (CONTEXT.CURRENT_STATE == Activity.INTREGRA_FOLHA_PROCESSO_PROC || CONTEXT.CURRENT_STATE == Activity.INTREGRA_FOLHA_PROCESSO_PROD) {
		if ($("#_nm_categoria").val() == "DOCUMENTACAO_PROCESSO"){
			$('#bt_integrarfol').show();
		}
    }
	
	if (CONTEXT.CURRENT_STATE == Activity.INTREGRA_FOLHA_PROCESSO_PROD) {
		if ($("#_nm_categoria").val() == "PBOM"){
			if ($("#nr_solicitacaoecr").val() == ""){
				$('#bt_integrarest').show();
				$('#bt_valestrut').show();
			}
		}
