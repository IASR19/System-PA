...
function createDataset(fields, constraints, sortFields) {
	...
	if (constraints != null) {
		getConstraintsValue(constraints
			, [ 'cd_filial', 'cd_centro_custo', 'cd_fornecedor']);
		...
		filter.cd_fornecedor =  constraintMap.get('cd_fornecedor');
	}
	...
	var constraints = [
		...
		, DatasetFactory.createConstraint("cd_fornecedor", filter.cd_fornecedor, filter.cd_fornecedor, ConstraintType.MUST)
	];
	var ds = DatasetFactory.getDataset("xcmg_adto_prestacao_viagem", FIELDS, constraints, null);
	if(ds != null && ds.rowsCount > 0){
		...
		var jaUsadasDs = DatasetFactory.getDataset("xcmg_adto_prestacao_viagem", FIELDS, constraintSolicsJaUsadas, null);
